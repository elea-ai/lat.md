...
- **Source code links**: Wiki links in `lat.md/` files can reference functions, classes, constants, and methods in TypeScript/JavaScript/Python/Rust/Go/C/Dart/SQL files. Use the full path: `[[src/config.ts#getConfigDir]]`, `[[src/server.ts#App#listen]]` (class method), `[[lib/utils.py#parse_args]]`, `[[src/lib.rs#Greeter#greet]]` (Rust impl method), `[[src/app.go#Greeter#Greet]]` (Go method), `[[src/app.h#Greeter]]` (C struct), `[[src/app.dart#Greeter#greet]]` (Dart method), `[[src/schema.sql#users#email]]` (SQL column). Can also link to folders (`[[src/components]]`) or files with any extension — these are validated to exist. Symbol references (`#`) only work for supported extensions. `lat check` validates these exist.
- **Code refs**: `// @lat: [[section-id]]` (JS/TS/Rust/Go/C/Dart), `# @lat: [[section-id]]` (Python), or `-- @lat: [[section-id]]` (SQL) — ties source code to concepts
